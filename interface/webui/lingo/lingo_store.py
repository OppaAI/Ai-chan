...
MATERIALS_VERSION = "2026.09.09-curated-banks"
...
def _seed_courses(con: sqlite3.Connection) -> None:
    """Legacy starter courses (import_curated_into is primary)."""
    ...
    con.execute(
        "INSERT OR IGNORE INTO courses(id,title,level,kind,cards_json) VALUES(?,?,?,?,?)",
                ...
                if has_kind:
                    ...
                    for front, back, reading, kind, level, used, ts in rows:
                        con.execute(
                            ...
                            "(front,back,reading,kind,level,used_count,created_at) VALUES(?,?,?,?,?,?,?)",
                    ...
                    for front, back, reading, level, used, ts in rows:
                        con.execute(
                            ...
                            "(front,back,reading,level,used_count,created_at) VALUES(?,?,?,?,?,?)",
                            (front, back, reading or "", normalize_level(level), used or 0, ts))
                        if con.total_changes:
                            moved["lesson_pool"] += 1
            finally:
                old.close()
